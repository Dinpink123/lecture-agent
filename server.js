...
app.post("/api/summarize", async (req, res) => {
  ...
  try {
    const SUMMARY_PROMPT = `אתה עוזר אקדמי שמסכם הרצאות אקדמיות בעברית בצורה מפורטת ומסודרת.
${manualCourse ? `שם הקורס שהוגדר ידנית: "${manualCourse}"` : "נסה לזהות את שם הקורס מהדיבור."}
${manualLesson ? `מספר השיעור: ${manualLesson}` : "נסה לזהות את מספר השיעור מהדיבור."}

העשר את התוכן: אם המרצה מזכיר מושגים, פסקי דין, חוקים, תיאוריות – הוסף הסברים קצרים מידע כללי.
...
החזר JSON בלבד ללא markdown:
{
  ...
  "quick_summary": "סיכום קצר של 5 שורות",
  "topics": [{"title": "נושא", "subtopics": [{"title": "תת-נושא", "bullets": [], "examples": [], "enrichment": "הרחבה מידע כללי"}]}],
  "definitions": [{"term": "מושג", "meaning": "הגדרה", "enrichment": "הרחבה"}],
  "cases_or_laws": [{"name": "שם", "context": "הקשר", "enrichment": "מידע נוסף"}],
  "exam_questions": [{"question": "שאלה", "hint": "רמז"}]
}`;
